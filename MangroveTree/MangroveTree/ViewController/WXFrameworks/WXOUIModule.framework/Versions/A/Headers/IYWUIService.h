 ...
 *   aPerson          YWPerson对象，当需要设置群的profile时，为nil
 *   aTribe           YWTribe对象，当需要设置单聊person的profile时，为nil，当需要设置群成员的profile时，与aPerson都不为nil
 *   aProgressBlock   获取到部分profile信息时，可以立即先调用这个block，通知IM，以便更快的显示这部分先获取到的信息。
 *   aCompletionBlock 获取profile完成后，调用这个block通知IM
 */
 ...
 *   aUserId 用户Id
 *   aProgressBlock 获取到部分profile信息时，可以先调用这个block，通知IM。以便更快的显示这部分先获取到的信息。
 *   aCompletionBlock 获取profile完成后，调用这个block通知IM
 */
