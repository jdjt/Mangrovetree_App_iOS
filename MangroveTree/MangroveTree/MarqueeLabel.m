...
- (void) scroll
{
    ...
    if (scrollDirection == AUTOSCROLL_SCROLL_LEFT){
        ...
        self.contentOffset = CGPointMake(label[0].frame.size.width+self.bufferSpaceBetweenLabels,0);
    }
...
- (void) readjustLabels
{
    ...
    for (int i = 0; i < NUM_LABELS; ++i){
        ...
        offset += label[i].frame.size.width + self.bufferSpaceBetweenLabels;
    }
    
    CGSize size;
    size.width = label[0].frame.size.width + self.frame.size.width + self.bufferSpaceBetweenLabels;
